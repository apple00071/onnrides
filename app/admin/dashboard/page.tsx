  ...
  try {
    ...
    return (
      <DashboardShell>
        ...
        <div className="bg-white rounded-xl border shadow-sm p-3 md:p-4 mb-2 md:mb-4">
          <div className="flex items-center justify-between mb-2 md:mb-3 pb-2 border-b border-gray-50">
            <h3 className="text-sm font-bold text-gray-900 tracking-tight">Today's Operations</h3>
            <span className="text-[10px] md:text-xs text-gray-500 font-medium">{new Date().toLocaleDateString('en-IN', { month: 'short', day: 'numeric' })}</span>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3">
            <div className="text-center p-2 rounded-lg bg-gray-50/50">
              <div className="text-lg md:text-xl font-bold text-blue-600 tabular-nums">{metrics.activeRentals}</div>
              <div className="text-[9px] font-semibold text-gray-500 uppercase tracking-wider mt-0.5">Active</div>
            </div>
            <div className="text-center p-2 rounded-lg bg-gray-50/50">
              <div className="text-lg md:text-xl font-bold text-green-600 tabular-nums">₹{metrics.todayRevenue.toLocaleString()}</div>
              <div className="text-[9px] font-semibold text-gray-500 uppercase tracking-wider mt-0.5">Revenue</div>
            </div>
            <div className="text-center p-2 rounded-lg bg-gray-50/50">
              <div className={`text-lg md:text-xl font-bold tabular-nums ${metrics.overdueReturns > 0 ? 'text-red-600' : 'text-green-600'}`}>
                {metrics.overdueReturns}
              </div>
              <div className="text-[9px] font-semibold text-gray-500 uppercase tracking-wider mt-0.5">Overdue</div>
            </div>
            <div className="text-center p-2 rounded-lg bg-gray-50/50">
              <div className={`text-lg md:text-xl font-bold tabular-nums ${metrics.availableVehicles < 3 ? 'text-orange-600' : 'text-green-600'}`}>
                {metrics.availableVehicles}
              </div>
              <div className="text-[9px] font-semibold text-gray-500 uppercase tracking-wider mt-0.5">Available</div>
            </div>
          </div>
